...
// Tarifas Anthropic por 1M tokens. Mismas que usa el orchestrator backend.
const PRICING_TABLE = [
  { model: 'claude-opus-4-6',   input: 15, output: 75, cache_write: 18.75, cache_read: 1.50 },
  { model: 'claude-sonnet-4-6', input: 3,  output: 15, cache_write: 3.75,  cache_read: 0.30 },
  { model: 'claude-haiku-4-5',  input: 1,  output: 5,  cache_write: 1.25,  cache_read: 0.10 },
];

function ApiCostPanel({ apiCost }) {
  const hasData = apiCost && apiCost.n_experiments_with_usage > 0;
  const totalInput = hasData ? apiCost.total_input_tokens + apiCost.total_cache_read + apiCost.total_cache_creation : 0;
  ...
  // Aproximación: blended rate $2/1M input.
  const estimatedSavings = hasData ? (apiCost.total_cache_read / 1_000_000) * 2 * 0.9 : 0;
    ...
    <div className="bg-white border border-slate-200 rounded-lg p-5">
      ...
      {!hasData ? (
        <div className="text-sm text-slate-500 mb-4">
          Aún no hay datos de uso. Corré un experimento para empezar a trackear costo real (los experimentos previos al deploy de hoy no tienen tracking).
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center mb-4">
            <Stat label="Costo total" value={`USD ${apiCost.total_cost_usd.toFixed(2)}`} />
            <Stat label="Costo promedio por experimento" value={`USD ${apiCost.avg_cost_per_experiment.toFixed(3)}`} />
            <Stat label="Tokens leídos del cache" value={fmtTokens(apiCost.total_cache_read)} positive={cachePct > 20} />
            <Stat label="% cache hit" value={`${cachePct.toFixed(1)}%`} positive={cachePct > 20} />
          </div>
          ...
          <table className="w-full text-xs">
            <thead className="text-slate-500">
              <tr>
                ...
                <th className="px-2 py-1 text-right">Cache write (1.25×)</th>
                <th className="px-2 py-1 text-right">Cache read (0.10×)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {PRICING_TABLE.map((p) => (
                <tr key={p.model}>
                  <td className="px-2 py-1.5 font-mono text-slate-700">{p.model}</td>
                  <td className="px-2 py-1.5 text-right">${p.input}</td>
                  <td className="px-2 py-1.5 text-right">${p.output}</td>
                  <td className="px-2 py-1.5 text-right text-slate-500">${p.cache_write}</td>
                  <td className="px-2 py-1.5 text-right text-emerald-700">${p.cache_read}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <p className="text-[10px] text-slate-400 mt-2">
            El sistema usa cache <b>ephemeral</b> (5 min TTL). Las librerías de nudges/templates/criteria + el system prompt se cachean
            por skill. La 2da corrida del día sobre el mismo producto cae en cache caliente y procesa el prefijo a 0.10× del precio normal.
          </p>
        </div>
      </details>
    </div>
